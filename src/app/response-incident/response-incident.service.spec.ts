...
describe('IncidentResponseService', () => {
  ...
  it('should return an error message when there is no token', () => {
    spyOn(service, 'getToken').and.returnValue(null); // Simular la ausencia del token

    service.getResponse(null, null).subscribe(response => {
      expect(response).toEqual([{ error: true, message: 'Error: No se pudo realizar la solicitud porque no hay token.' }]);
    });
  });

  it('should send a POST request with the correct headers and body when token is present', () => {
    const mockToken = 'mock-token';
    const mockTitulo = 'test-title';
    const mockTipo = 1;
    const mockResponse = { success: true };

    spyOn(service, 'getToken').and.returnValue(mockToken); // Simular la obtención del token

    service.getResponse(mockTitulo, mockTipo).subscribe(response => {
      expect(response).toEqual(mockResponse);
    });

    const req = httpMock.expectOne(`${environment.incidentURL}/response-indicent`);
    expect(req.request.method).toBe('POST');
    expect(req.request.headers.get('Authorization')).toBe(`Bearer ${mockToken}`);
    expect(req.request.headers.get('Content-Type')).toBe('application/json');
    expect(req.request.body).toEqual({ titulo: mockTitulo, tipo: mockTipo });
